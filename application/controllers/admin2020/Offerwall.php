...
class Offerwall extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model($this->path_to_view_admin . 'Offer_model', 'offer');
    }
    ...
    function setDatatableOffer() {
        $requestData = $_REQUEST;
        $columns = array(            
            2 => 'offer_name',
            3 => 'offer_image',
            4 => 'offer_actual_price',
            5 => 'offer_selling_price',
            6 => 'offer_status',
            7 => 'date_created'
        );
        $totalData = $this->offer->get_list_count_product();
        $totalFiltered = $totalData;

        $sql = "SELECT offer_id,offer_name,offer_image,offer_actual_price,offer_selling_price,offer_status,date_created FROM offer";
        
        if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
            $sql .= " WHERE  offer_id LIKE '%" . $requestData['search']['value'] . "%' ";
            $sql .= " OR  offer_name LIKE '%" . $requestData['search']['value'] . "%' ";
            $sql .= " OR  offer_image LIKE '%" . $requestData['search']['value'] . "%' ";
            $sql .= " OR  offer_actual_price LIKE '%" . $requestData['search']['value'] . "%' ";
            $sql .= " OR  offer_selling_price LIKE '%" . $requestData['search']['value'] . "%' ";
            $sql .= " OR  offer_status LIKE '%" . $requestData['search']['value'] . "%' ";
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $nestedData[] = $i;
            $nestedData[] = $row['offer_name'];
            $nestedData[] = '<img src="' . base_url() . $this->product_image . "thumb/100x100_" . $row['offer_image'] . '">';
            $nestedData[] = sprintf('%.' . $this->functions->getCurrencyDecimal($this->system->currency) . 'F', $row['offer_actual_price']);
            $nestedData[] = sprintf('%.' . $this->functions->getCurrencyDecimal($this->system->currency) . 'F', $row['offer_selling_price']);
            if ($this->system->demo_user == 1 && $row['offer_id'] <= 2) {
